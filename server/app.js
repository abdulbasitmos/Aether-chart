...
// POST /api/upload — upload device file (base64 image or video)
app.post('/api/upload', authenticateToken, async (req, res) => {
  const { fileData, fileName, mimeType } = req.body;
  if (!fileData) return res.status(400).json({ error: 'fileData base64 string is required' });

  try {
    const matches = fileData.match(/^data:(.+);base64,(.+)$/);
    let buffer;
    let detectedMime = mimeType || 'image/png';
    let name = fileName || `file_${Date.now()}.png`;
    ...
    const result = await uploadFile(buffer, name, detectedMime);
...
app.get('/api/status', authenticateToken, async (req, res) => {
  try {
    ...
    // Fetch statuses only for current user and their contacts
    const statuses = await Status.find({
      userId: { $in: [...contactIds, req.userId] }
    })
      .populate('userId', 'name username avatar contacts')
      .populate('items.views', 'name username avatar');
    ...
    for (const statusDoc of statuses) {
      if (!statusDoc.userId) continue;

      // Filter: Mutual contact privacy check
      const isOwner = statusDoc.userId._id.toString() === req.userId.toString();
      const contactHasMe = statusDoc.userId.contacts && 
                           statusDoc.userId.contacts.some(cId => cId.toString() === req.userId.toString());
      
      // If not owner and not mutual contact, skip
      if (!isOwner && !contactHasMe) {
        continue;
      }
...
// 14. Meta-Style Smart AI Assistant Chat Route (Grok API Integration)
app.post('/api/ai/chat', authenticateToken, async (req, res) => {
  const { prompt, history } = req.body;
  const apiKey = process.env.GROK_API_KEY;

  if (!apiKey || apiKey.includes('placeholder') || apiKey.length < 15) {
    return res.status(400).json({ 
      error: 'Grok API Key is not configured or is a placeholder. Please define GROK_API_KEY in your server/.env file or use a client-side API Key in Settings.' 
    });
  }
  ...
  try {
    ...
    if (history && Array.isArray(history)) {
      history.forEach(msg => {
        const role = msg.senderId === 'user_me' ? 'user' : 'assistant';
    ...
    let endpoint = 'https://api.xai.com/v1/chat/completions';
    let primaryModel = 'grok-beta';
    let fallbackModel = 'grok-2-latest';
    ...
    console.log(`[AI Route] Calling API at ${endpoint} with model ${primaryModel} and prompt:`, prompt);
    ...
    if (!response.ok) {
      console.log(`[AI Route] Primary request failed (status: ${response.status}). Retrying with fallback model ${fallbackModel}...`);
    ...
    console.log('[AI Route] API response status:', response.status);

    if (!response.ok) {
      const errText = await response.text();
      console.error('[AI Route] API communication failure:', errText);
      return res.status(response.status).json({ error: 'Failed to communicate with AI API' });
    }

    const data = await response.json();
    const responseText = data.choices?.[0]?.message?.content || "I was unable to retrieve a response from my neural core.";
    
    // Check if Grok determined it's an image generation command
...
io.on('connection', (socket) => {
  ...
  // ── BUSINESS REAL-TIME EVENTS ──────────────────────────────────────────

  // Notify business owner when a product is purchased
  socket.on('business_purchase', async (data) => {
    if (!socket.userId) return;
    const { businessOwnerId, productName, quantity, total, buyerName } = data;
    const ownerSocketId = activeSockets.get(businessOwnerId);
    if (ownerSocketId) {
      io.to(ownerSocketId).emit('business_purchase_notification', {
        type: 'purchase',
        productName,
        quantity,
        total,
  ...
  // Notify business owner when someone RSVPs to their event
  socket.on('business_event_rsvp', async (data) => {
    if (!socket.userId) return;
    const { businessOwnerId, eventTitle, userName } = data;
    const ownerSocketId = activeSockets.get(businessOwnerId);
    if (ownerSocketId) {
      io.to(ownerSocketId).emit('business_event_notification', {
        type: 'event_rsvp',
        eventTitle,
        userName,
  ...
  // Notify business owner when someone enrolls in their workshop
  socket.on('business_workshop_enroll', async (data) => {
    if (!socket.userId) return;
    const { businessOwnerId, workshopTitle, userName } = data;
    const ownerSocketId = activeSockets.get(businessOwnerId);
    if (ownerSocketId) {
      io.to(ownerSocketId).emit('business_workshop_notification', {
        type: 'workshop_enroll',
        workshopTitle,
        userName,
        userId: socket.userId,
        timestamp: new Date().toISOString()
  ...
  // Notify user event creator when someone RSVPs
  socket.on('user_event_rsvp', async (data) => {
    if (!socket.userId) return;
    const { eventCreatorId, eventTitle, rsvpStatus, userName } = data;
    const creatorSocketId = activeSockets.get(eventCreatorId);
    if (creatorSocketId) {
      io.to(creatorSocketId).emit('user_event_notification', {
        type: 'rsvp',
        eventTitle,
        rsvpStatus,
        userName,
  ...
  // Workshop invite: send invite notification to specific user
  socket.on('workshop_invite_user', async (data) => {
    if (!socket.userId) return;
    const { targetUserId, workshopTitle, inviteLink, businessName } = data;
    const targetSocketId = activeSockets.get(targetUserId);
    if (targetSocketId) {
      io.to(targetSocketId).emit('workshop_invite_received', {
        workshopTitle,
        inviteLink,
        businessName,
    ...
    // Also save a persistent notification
    try {
      await new Notification({
        userId: targetUserId,
        type: 'contact_request',
        title: `🎓 Workshop Invite: ${workshopTitle}`,
        content: `${businessName} invited you to join their workshop. Click to enroll!`,
        fromUserId: socket.userId
      }).save();
    } catch (err) {
      console.error('Failed to save workshop invite notification:', err.message);
    }
  });

  // ── PUBLIC FEED REAL-TIME EVENTS ─────────────────────────────────────
  
  // Broadcast new post to all online users
  socket.on('feed_new_post', (data) => {
    io.emit('feed_post_created', data);
  });

  // Broadcast post like update
  socket.on('feed_like_toggle', (data) => {
    io.emit('feed_post_liked', data);
  });

  // Broadcast new comment
  socket.on('feed_new_comment', (data) => {
    io.emit('feed_comment_added', data);
  });
